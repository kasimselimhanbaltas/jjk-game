...
export default megunaSlice;

export const toggleSimpleDomainCD = (): AppThunk => (dispatch, getState) => {
    const state = getState();
    if (!state.MegunaState.simpleDomain.skill.isReady) return;
    const cooldown = state.MegunaState.simpleDomain.skill.cooldown;
    dispatch(
        setSimpleDomain({
            isActive: false,
            duration: state.MegunaState.simpleDomain.duration,
            skill: {
                isReady: false,
                cooldown: cooldown,
    ...
    const interval = setInterval(() => {
        ...
        const remainingTime =
            currentState.MegunaState.simpleDomain.skill.remainingTime;
        if (remainingTime > 1) {
...
export const toggleCleaveCD = (): AppThunk => (dispatch, getState) => {
    const state = getState();
    if (!state.MegunaState.cleaveCD.isReady) return;
    const cooldown = state.MegunaState.cleaveCD.cooldown;
    dispatch(
        setCleaveCD({
            isReady: false,
            remainingTime: cooldown,
    ...
    const interval = setInterval(() => {
        const currentState = getState();
        const remainingTime = currentState.MegunaState.cleaveCD.remainingTime;
        if (remainingTime > 1) {
            dispatch(
                setCleaveCD({
                    isReady: false,
...
export const toggleDismantleCD = (): AppThunk => (dispatch, getState) => {
    const state = getState();
    if (!state.MegunaState.dismantleCD.isReady) return;
    const cooldown = state.MegunaState.dismantleCD.cooldown;
    dispatch(
        setDismantleCD({
            isReady: false,
            remainingTime: cooldown,
    ...
    const interval = setInterval(() => {
        const currentState = getState();
        const remainingTime = currentState.MegunaState.dismantleCD.remainingTime;
        if (remainingTime > 1) {
            dispatch(
                setDismantleCD({
                    isReady: false,
...
export const toggleDomainCD = (): AppThunk => (dispatch, getState) => {
    const state = getState();
    if (!state.MegunaState.domainCD.isReady) return;
    const cooldown = state.MegunaState.domainCD.cooldown;
    dispatch(
        setDomainCD({
            isReady: false,
            remainingTime: cooldown,
    ...
    const interval = setInterval(() => {
        const currentState = getState();
        const remainingTime = currentState.MegunaState.domainCD.remainingTime;
        if (remainingTime > 1) {
            dispatch(
                setDomainCD({
                    isReady: false,
